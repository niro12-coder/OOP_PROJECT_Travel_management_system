...
public class hotelReservation extends Hotel implements Serializable {
        ...
        if (ticket.getNumberOfSeats() == 1) {
                ...
                if(Agency.hotels.get(i).getHotelLocation().equals(ticket.getTrip().getDestination())){
                    for (int j = 0; i < Agency.hotels.get(i).getNumberOfSingleRooms(); j++)
                    {
                        ...
                        if(reservationArraySize==0){
                            availableHotels.add(new hotelReservation(Agency.hotels.get(i)));
                            availableHotels.get(avaHotels).assignAvailableRoom(Agency.hotels.get(i).singleRoom.get(j),tripStartDate,tripEndDate);
                            avaHotels++;
                            break;
                        }
                        else {
                            for (int k = 0; k < reservationArraySize; k+=2) {
                                ...
                                if( ((k==0)&&(tripEndDate.isBefore( Agency.hotels.get(i).singleRoom.get(j).Reservations.get(k))))||
                                        (tripStartDate.isAfter( Agency.hotels.get(i).singleRoom.get(j).Reservations.get(k))&&
                                                (tripEndDate.isBefore( Agency.hotels.get(i).singleRoom.get(j).Reservations.get(k+1))))||
                                        ((k== reservationArraySize-1) &&(tripStartDate.isAfter( Agency.hotels.get(i).singleRoom.get(j).Reservations.get(k)))) ){
                                    availableHotels.add(new hotelReservation(Agency.hotels.get(i)));
                                    availableHotels.get(avaHotels).assignAvailableRoom(Agency.hotels.get(i).singleRoom.get(j),tripStartDate,tripEndDate);
                                    avaHotels++;
                                    break;
            ...
            for (int i = 0; i < Agency.hotels.size(); i++) {
                if(Agency.hotels.get(i).getHotelLocation().equals(ticket.getTrip().getDestination())){
                    for (int j = 0; i < Agency.hotels.get(i).getNumberOfDoubleRooms(); j++)
                    {
                        ...
                        if(reservationArraySize==0){
                            availableHotels.add(new hotelReservation(Agency.hotels.get(i)));
                            availableHotels.get(avaHotels).assignAvailableRoom(Agency.hotels.get(i).doubleRoom.get(j),tripStartDate,tripEndDate);
                            avaHotels++;
                            break;
                        }
                        else {
                            for (int k = 0; k < reservationArraySize; k+=2) {
                                ...
                                //case 3 the date is in between
                                //in this case we check diTimes
                                if( ((k==0)&&(tripEndDate.isBefore( Agency.hotels.get(i).doubleRoom.get(j).Reservations.get(k))))||
                                        (tripStartDate.isAfter( Agency.hotels.get(i).doubleRoom.get(j).Reservations.get(k))&&
                                                (tripEndDate.isBefore( Agency.hotels.get(i).doubleRoom.get(j).Reservations.get(k+1))))||
                                        ((k== reservationArraySize-1) &&(tripStartDate.isAfter( Agency.hotels.get(i).doubleRoom.get(j).Reservations.get(k)))) ){
                                    availableHotels.add(new hotelReservation(Agency.hotels.get(i)));
                                    availableHotels.get(avaHotels).assignAvailableRoom(Agency.hotels.get(i).doubleRoom.get(j),tripStartDate,tripEndDate);
                                    avaHotels++;
                                    break;
            ...
            for (int i = 0; i < Agency.hotels.size(); i++) {
                ...
                if(Agency.hotels.get(i).getHotelLocation().equals(ticket.getTrip().getDestination())){
                    ...
                    {
                        int reservationArraySize= Agency.hotels.get(i).familyRoom.get(j).Reservations.size();
                        if(reservationArraySize==0){
                            if(counter==0){ availableHotels.add(new hotelReservation(Agency.hotels.get(i)));}

                            availableHotels.get(avaHotels).assignAvailableRoom(Agency.hotels.get(i).familyRoom.get(j),tripStartDate, tripEndDate,counter);
                            counter++;
                            ...
                            for (int k = 0; k < reservationArraySize; k+=2) {
                                ...
                                //case 3 the date is in between
                                //in this case we check diTimes
                                if(  ((k==0)&&(tripEndDate.isBefore( Agency.hotels.get(i).familyRoom.get(j).Reservations.get(k))))||
                                        (tripStartDate.isAfter( Agency.hotels.get(i).familyRoom.get(j).Reservations.get(k))&&
                                                (tripEndDate.isBefore( Agency.hotels.get(i).familyRoom.get(j).Reservations.get(k+1))))||
                                        ((k== reservationArraySize-1) &&(tripStartDate.isAfter( Agency.hotels.get(i).familyRoom.get(j).Reservations.get(k)))) )
                                {
                                    if(counter==0){ availableHotels.add(new hotelReservation(Agency.hotels.get(i)));}
                                    availableHotels.get(avaHotels).assignAvailableRoom(Agency.hotels.get(i).familyRoom.get(j),tripStartDate,tripEndDate,counter);
                                    counter++;
    ...
    public void displayHotelForBooking(int i , boolean familyRoom,int numberOfSeats, boolean confirmation){
        ...
        if(!confirmation){
            if(i<9){System.out.println("[0"+ (i+1)+"] " + this.getHotelName());}
            else {System.out.println("["+(i+1)+"] " + this.getHotelName());}
        }else {
...
// meal[0]=3-1=2
    ...
    public int numberOfRoomsRequired(int numberOfSeats){
            ...
            } else { numberOfRoomsRequired=(numberOfSeats/ roomAvailable.get(0).getRoomLimit())+1; }
                ...
                switch(ticket.getNumberOfSeats()){
                    case 1:
                        for(int r = 0; r< Agency.hotels.get(h).getNumberOfSingleRooms(); r++){
                            if(Agency.hotels.get(h).singleRoom.get(r).getRoomId().equals(ticket.Hotel.roomAvailable.get(0).getRoomId())){
                                Agency.hotels.get(h).singleRoom.get(r).addNewReservation(ticket.Hotel.startDate, ticket.Hotel.endDate);
                                break;
                            }
                        }
                        break;
                    case 2:
                        for(int r = 0; r< Agency.hotels.get(h).getNumberOfDoubleRooms(); r++){
                            if(Agency.hotels.get(h).doubleRoom.get(r).getRoomId().equals(ticket.Hotel.roomAvailable.get(0).getRoomId())){
                                Agency.hotels.get(h).doubleRoom.get(r).addNewReservation(ticket.Hotel.startDate, ticket.Hotel.endDate);
                                break;
                            }
                        }
                        break;
                    default:
                        for(int n=0;n< ticket.Hotel.roomAvailable.size();n++){
                            for(int r = 0; r< Agency.hotels.get(h).getNumberOfFamilyRooms(); r++){
                                if(Agency.hotels.get(h).familyRoom.get(r).getRoomId().equals(ticket.Hotel.roomAvailable.get(n).getRoomId())){
                                    Agency.hotels.get(h).familyRoom.get(r).addNewReservation(ticket.Hotel.startDate, ticket.Hotel.endDate);
                                    break;
                                }
                            }
                        }
            ...
            if(Agency.hotels.get(h).getHotelID().equals(ticket.Hotel.getHotelID())){
                switch(ticket.getNumberOfSeats()){
                    case 1:
                        for(int r = 0; r< Agency.hotels.get(h).getNumberOfSingleRooms(); r++){
                            if(Agency.hotels.get(h).singleRoom.get(r).getRoomId().equals(ticket.Hotel.roomAvailable.get(0).getRoomId())){
                                Agency.hotels.get(h).singleRoom.get(r).Reservations.remove(tripStartDate);
                                 Agency.hotels.get(h).singleRoom.get(r).Reservations.remove(tripEndDate);
                                 break;
                            }
                        }
                        break;
                    case 2:
                        for(int r = 0; r< Agency.hotels.get(h).getNumberOfDoubleRooms(); r++){
                            if(Agency.hotels.get(h).doubleRoom.get(r).getRoomId().equals(ticket.Hotel.roomAvailable.get(0).getRoomId())){
                                Agency.hotels.get(h).doubleRoom.get(r).Reservations.remove(tripStartDate);
                                Agency.hotels.get(h).doubleRoom.get(r).Reservations.remove(tripEndDate);
                                break;
                            }
                        }
                        break;
                    default:
                        for(int n=0;n< ticket.Hotel.roomAvailable.size();n++){
                            for(int r = 0; r< Agency.hotels.get(h).getNumberOfFamilyRooms(); r++){
                                if(Agency.hotels.get(h).familyRoom.get(r).getRoomId().equals(ticket.Hotel.roomAvailable.get(n).getRoomId())){
                                    Agency.hotels.get(h).familyRoom.get(r).Reservations.remove(tripStartDate);
                                    Agency.hotels.get(h).familyRoom.get(r).Reservations.remove(tripEndDate);
                                    break;
                                }
                            }
                        }
